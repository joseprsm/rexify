...
class QueryModelTest(tf.test.TestCase):
    def setUp(self):
        ...
        self.inputs = list(self.inputs.batch(1).take(1))[0]["query"]
        self.model = QueryModel(
            n_users=self.feat.model_params["n_unique_users"],
            user_id=self.feat.model_params["user_id"],
            layer_sizes=[64, 32],
        )
    ...
    def testConfig(self):
        config = {
            "n_items": self.feat.model_params["n_unique_users"],
            "user_id": self.feat.model_params["user_id"],
            "embedding_dim": 32,
            "layer_sizes": [64, 32],
        }
