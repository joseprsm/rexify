...
class LookupGenExecutorTest(tf.test.TestCase):
    ...
    def testGetLookupParams(self):
        params = executor.Executor.get_lookup_params(mock_candidates, mock_recommender, 'candidate_model', 'itemId')
        self.assertEqual(len(params[0]), len(mock_candidates))
        self.assertEqual(params[1].shape[-1], 32)

    @mock.patch.multiple(
        executor,
        _get_examples=get_mock_candidates,
        _get_recommender=get_mock_model)
    def testDo(self):
