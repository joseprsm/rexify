...
def read_examples(
    example_uri: Union[str, bytes, os.PathLike], feature_spec: Dict[Text, Any]
) -> tf.data.Dataset:
    def parse_examples(x):
        return tf.io.parse_example(x, features=feature_spec)

    filenames = [
        os.path.join(example_uri, filename)
        for filename in tf.io.gfile.listdir(example_uri)
    ]
...
def get_feature_spec(schema: Dict[Text, Text]) -> Dict[Text, Any]:
    return {key: tf.io.FixedLenFeature([], dtype=tf.int64) for key in schema.keys()}
...
def load_model(artifact_list: List[Artifact], model_name: Text = "Format-Serving"):
    ...
    model: tf.keras.Model = tf.keras.models.load_model(
        os.path.join(model_uri, model_name)
    )
    return model
...
def export_model(
    artifact_list: List[Artifact], model: tf.keras.Model, model_name: Text, **kwargs
):
